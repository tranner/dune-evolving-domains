...
struct CoupledHeatModel : public DiffusionModel<FunctionSpace,GridPart>
{
  ...
  typedef TemporalCoupledProblemInterface< FunctionSpace > ProblemType;
  ...
  CoupledHeatModel( const ProblemType& problem,
			  ...
			  JacobianRangeType &flux ) const
  {
    ...
    flux = 0;
    for( unsigned int i = 0; i < flux.columns(); ++i )
      {
	for( unsigned int k = 0; k < x.size(); ++k )
	  flux[ i ][ 0 ] += D[ i ][ k ] * gradient[ k ][ 0 ] + b[ i ] * value;
      }
    flux *= timeStepFactor_ * timeProvider_.deltaT();
  }
